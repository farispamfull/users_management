...
class Test01UserAPI:
    ...
    def test_02_users_username_not_auth(self, client, admin):
        ...
        assert response.status_code == 401, (
            'Проверьте, что при GET запросе `/api/v1/users/{id}/` без токена авторизации возвращается статус 401'
        )
    ...
    def test_06_users_username_get_auth(self, user_client, admin):
        ...
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/users/{id}/` с токеном авторизации возвращается статус 200'
        )
        response_data = response.json()
        ...
        response = user_client.get(f'/api/v1/users/{moderator.id}/')
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/users/{id}/` с токеном авторизации возвращается статус 200'
        )
        response_data = response.json()
    ...
    def test_07_users_username_patch_auth(self, user_client, admin):
        ...
        response = user_client.patch(f'/api/v1/users/{admin.id}/', data=data)
        assert response.status_code == 200, (
            'Проверьте, что при PATCH запросе `/api/v1/users/{id}/` '
            'с токеном авторизации возвращается статус 200'
        )
        test_admin = get_user_model().objects.get(email=admin.email)
        assert test_admin.first_name == data['first_name'], (
            'Проверьте, что при PATCH запросе `/api/v1/users/{id}/` изменяете данные.'
        )
    ...
    @pytest.mark.django_db(transaction=True)
    def test_08_users_delete_auth(self, user_client):
        user, moderator = create_users_api(user_client)
        response = user_client.delete(f'/api/v1/users/{user.id}/')
        assert response.status_code == 204, (
            'Проверьте, что при DELETE запросе `/api/v1/users/{id}/` возвращаете статус 204'
        )
        assert get_user_model().objects.count() == 2, (
            'Проверьте, что при DELETE запросе `/api/v1/users/{id}/` удаляете пользователя'
        )
